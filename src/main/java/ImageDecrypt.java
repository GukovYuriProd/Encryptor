...
public class ImageDecrypt {
    public static void main (String[] args) throws IOException {
        DecryptImageBySPBlock("src/main/resources/output.bmp", "src/main/resources/decrypted_out.bmp");
    }
    public static void DecryptImageBySPBlock (String source, String destination) throws IOException {
        byte[] bitmap = ReadBytes(source); //чтение
        bitmap = decrypted_bitmap(bitmap); //шифрование
    ...
    public static byte[] decrypted_bitmap (byte[] bitmap){
        ...
        for (int index = 54; index < pixels_count; index += data_block){
            //здесь на каждой итерации обрабатываем 6 последовательных байтов в значимой массе байтов
            //и заполняем их биты в общий битовый блок bits на 48 разрядов
    ...
    public static byte[] BlockDecrypt(byte[] block){
        byte[] output = block;
        int rounds = 5;
        for (int index = rounds; index > 0; index--){
            output = P_BlockOperation(output, 46-index);
